...
def initial_tiling(img):
    ...
    for i in range(arr.shape[0]//tile_size):
        for j in range(arr.shape[1]//tile_size):
            ...
            dic[c]=set([t*step_size+k for t in range(tile_size*i,tile_size*(i+1)) for k in range(tile_size*j,tile_size*(j+1))])
        ...
        if col_step<col_misfit_index:
            for i in range(col_misfit_index//col_step):
                if not i==col_misfit_index//col_step-1:
                    ...
                    dic[c]=set([t*step_size+k for t in range(row_misfit_index,arr.shape[0]) for k in range(col_step*i,col_step*(i+1))])
                    ...
                    dic[c]=set([t*step_size+k for t in range(row_misfit_index,arr.shape[0]) for k in range(col_step*i,col_misfit_index)])
            ...
            dic[c]=set([t*step_size+k for t in range(row_misfit_index,arr.shape[0]) for k in range(col_misfit_index)])
        ...
        if row_step<row_misfit_index:
            for i in range(row_misfit_index//row_step):
                if not i== row_misfit_index//row_step-1:
                    ...
                    dic[c]=set([t*step_size+k for t in range(row_step*i,row_step*(i+1)) for k in range(col_misfit_index,arr.shape[1])])
                    ...
                    dic[c]=set([t*step_size+k for t in range(row_step*i,row_misfit_index) for k in range(col_misfit_index,arr.shape[1])])
            ...
            # dic[c]=arr[:row_misfit_index,col_misfit_index:].flatten()
            dic[c]=set([t*step_size+k for t in range(row_misfit_index) for k in range(col_misfit_index,arr.shape[1])])
    ...
    if arr.shape[0]%tile_size!=0 and arr.shape[1]%tile_size!=0 :
        # dic[c]=arr[row_misfit_index:,col_misfit_index:].flatten()
        dic[c]=set([t*step_size+k for t in range(row_misfit_index,arr.shape[0]) for k in range(col_misfit_index,arr.shape[1])])
...
def write_txt_report(include_accuracy=False):
    np.set_printoptions(suppress=True)
    number_pixel_in_dic=0
    for cluster in dic:
        number_pixel_in_dic+=len(dic[cluster])
    file = open(r"E:\Vitaly Vanchurin\report.txt", "w")
    file.write(f"Total number of pixels were...{total_pixel_num}\n")
    file.write(f"Total pixels present in dictionary are..{number_pixel_in_dic}\n")
    file.write(f"Orphan pixel numbers are..{total_pixel_num-number_pixel_in_dic}\n")
    ...
    for cluster in ref_dic:
        file.write(
            f"This cluster has {len(dic[cluster])} members.\n "
            f"{np.around(ref_dic[cluster],decimals=3)}\n"
            f"\n --------------------------------------\n")
